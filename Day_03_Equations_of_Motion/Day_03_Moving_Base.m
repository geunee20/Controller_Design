%% variablees
syms x(t) theta1(t) theta2(t) theta3(t) l1 l2 l3 m0 m1 m2 m3 g
...
% Linear Displacement
s0 = [x, 0].';
s1 = [x + 1/2*l1*C1, 1/2*l1*S1].';
s2 = [x + l1*C1 + 1/2*l2*C12, l1*S1 + 1/2*l2*S12].';
s3 = [x + l1*C1 + l2*C12 + 1/2*l3*C123, l1*S1 + l2*S12 + 1/2*l3*S123].';
...
L = KE0 + KE1 + KE2 + KE3 - PE0 - PE1 - PE2 - PE3;
...
%% Jacobian
J = jacobian([s3; theta1+theta2+theta3], [x, theta1, theta2, theta3])